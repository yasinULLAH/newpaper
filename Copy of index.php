...
// If connection fails, create the database and reconnect
...
// Seed initial data if no admin user exists
$admin_check = $conn->query("SELECT COUNT(*) as count FROM users WHERE role = 'admin'")->fetch_assoc();
if ($admin_check['count'] == 0) {
    $admin_password = password_hash('admin123', PASSWORD_DEFAULT);
    $conn->query("INSERT INTO users (username, email, password, role) VALUES ('admin', 'admin@newspaper.pk', '$admin_password', 'admin')");
    $categories = [
    ...
    foreach ($sample_articles as $article) {
        ...
        $stmt = $conn->prepare("INSERT INTO articles (title_en, title_ur, content_en, content_ur, category_id, author_id, is_breaking, slug, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssiiiss", $article['title_en'], $article['title_ur'], $article['content_en'], $article['content_ur'], $category_id, $admin_user_id, $article['is_breaking'], $slug, $article['image']);
        $stmt->execute();
    }
}
// Language handling
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_article']) && canEdit()) {
        ...
        if (empty($title_en) || empty($title_ur) || empty($content_en) || empty($content_ur) || empty($category_id)) {
            ...
            $stmt = $conn->prepare("INSERT INTO articles (title_en, title_ur, content_en, content_ur, category_id, author_id, is_breaking, slug, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssiiiss", $title_en, $title_ur, $content_en, $content_ur, $category_id, $_SESSION['user_id'], $is_breaking, $slug, $image_path);
            if ($stmt->execute()) {
    ...
    if (isset($_POST['update_article']) && canEdit()) {
        ...
        if (empty($title_en) || empty($title_ur) || empty($content_en) || empty($content_ur) || empty($category_id)) {
            ...
            $stmt = $conn->prepare("UPDATE articles SET title_en = ?, title_ur = ?, content_en = ?, content_ur = ?, category_id = ?, is_breaking = ?, status = ?, slug = ?, image = ? WHERE id = ?");
            $stmt->bind_param("ssssiissis", $title_en, $title_ur, $content_en, $content_ur, $category_id, $is_breaking, $status, $slug, $image_path, $article_id_to_update);
            if ($stmt->execute()) {
    ...
    if (isset($_POST['delete_category']) && hasRole('admin')) {
        ...
        // Articles related to this category will have their category_id set to NULL due to ON DELETE SET NULL
    ...
    if (isset($_POST['add_comment']) && isLoggedIn()) {
        ...
        $email = ''; // Not used when logged in, can fetch from user table if needed
        if (empty($comment)) {
            $error = ($lang === 'ur' ? 'تبصرہ خالی نہیں ہو سکتا۔' : 'Comment cannot be empty.');
        } else {
            $stmt = $conn->prepare("INSERT INTO comments (article_id, user_id, name, comment, status) VALUES (?, ?, ?, ?, 'pending')");
            $stmt->bind_param("iiss", $article_id, $user_id, $name, $comment);
            if ($stmt->execute()) {
        ...
        if (isset($_FILES['backup_file']) && $_FILES['backup_file']['error'] == UPLOAD_ERR_OK) {
            ...
            if ($data) {
                ...
                foreach ($data['users'] as $user) {
                    $stmt = $conn->prepare("INSERT INTO users (id, username, email, password, role, created_at) VALUES (?, ?, ?, ?, ?, ?)");
                    $stmt->bind_param("isssss", $user['id'], $user['username'], $user['email'], $user['password'], $user['role'], $user['created_at']);
                    $stmt->execute();
                }
                // Import categories
                ...
                foreach ($data['articles'] as $article) {
                    $stmt = $conn->prepare("INSERT INTO articles (id, title_en, title_ur, content_en, content_ur, category_id, author_id, image, is_breaking, views, status, slug, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    $stmt->bind_param("issssiiissssss", $article['id'], $article['title_en'], $article['title_ur'], $article['content_en'], $article['content_ur'], $article['category_id'], $article['author_id'], $article['image'], $article['is_breaking'], $article['views'], $article['status'], $article['slug'], $article['created_at'], $article['updated_at']);
                    $stmt->execute();
                }
                // Import comments
                foreach ($data['comments'] as $comment) {
                    $stmt = $conn->prepare("INSERT INTO comments (id, article_id, user_id, name, email, comment, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                    $stmt->bind_param("iiisssss", $comment['id'], $comment['article_id'], $comment['user_id'], $comment['name'], $comment['email'], $comment['comment'], $comment['status'], $comment['created_at']);
                    $stmt->execute();
                }
                // Import tags
...
// Handle GET requests for logout, views, and RSS feed
...
if (isset($_GET['action']) && $_GET['action'] === 'export' && hasRole('admin')) {
    ...
    $tables_to_export = ['users', 'categories', 'articles', 'comments', 'tags', 'article_tags'];
    $export_data = [];
...
if ($view === 'article' && $article_id) {
    // Increment views using prepared statement for security
...
<head>
    ...
    <meta name="description" content="Complete Pakistani Newspaper - Latest News in Urdu and English. Breaking news, sports, technology, and business updates from Pakistan.">
    <meta name="keywords" content="Pakistan News, Urdu News, English News, Breaking News, Sports, Politics, Technology, Business, Entertainment, Opinion, Pakistan Times, Yasin Ullah">
    <title><?= $lang === 'ur' ? 'پاکستان ٹائمز - تازہ خبریں، اردو اور انگریزی میں' : 'Pakistan Times - Latest News in Urdu and English' ?></title>
...
<body>
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto">
                    ...
                    ?>
                        <li class="nav-item">
                            <a class="nav-link" href="?view=category&cat=<?= $cat['slug'] ?>&lang=<?= $lang ?>">
    ...
    <?php
    $breaking_news_query = $conn->query("SELECT * FROM articles WHERE is_breaking = 1 AND status = 'published' ORDER BY created_at DESC LIMIT 5");
            ...
            <div class="col-lg-8">
                ...
                function include_home_view()
                {
                    ...
                    while ($article = $articles->fetch_assoc()) {
                        ...
                        echo '<h5 class="card-title mt-2">' . htmlspecialchars($title) . '</h5>';
                        ...
                        echo '<i class="fas fa-calendar me-1"></i>' . date('M j, Y', strtotime($article['created_at'])) . ' • ';
                    ...
                    echo '</div>';
                    $total_articles_query = $conn->query("SELECT COUNT(*) as count FROM articles WHERE status = 'published'");
                ...
                function include_article_view()
                {
                    ...
                    echo '<span class="category-badge">' . htmlspecialchars($category) . '</span>';
                    echo '<h1 class="card-title mt-3 mb-3">' . htmlspecialchars($title) . '</h1>';
                    echo '<div class="meta-info mb-4">';
                    echo '<i class="fas fa-user me-2"></i>' . htmlspecialchars($article['username'] ?? ($lang === 'ur' ? 'نامعلوم مصنف' : 'Unknown Author')) . ' • ';
                    echo '<i class="fas fa-calendar me-2"></i>' . date('M j, Y g:i A', strtotime($article['created_at'])) . ' • ';
                ...
                function include_category_view()
                {
                    ...
                    echo '</div>';
                    $total_articles_cat_query = $conn->prepare("SELECT COUNT(*) as count FROM articles WHERE category_id = ? AND status = 'published'");
                ...
                function include_search_view()
                {
                    ...
                    $query = trim($_GET['q'] ?? '');
                    if (empty($query)) {
                        echo '<div class="alert alert-warning" role="alert">' . ($lang === 'ur' ? 'تلاش کی شرائط داخل کریں' : 'Please enter search terms') . '</div>';
                        return;
                    }
                    echo '<h2 class="mb-4">' . ($lang === 'ur' ? 'تلاش کے نتائج برائے: ' : 'Search Results for: ') . htmlspecialchars($query) . '</h2>';
                    ...
                    $stmt = $conn->prepare("
                        ...
                        LEFT JOIN categories c ON a.category_id = c.id 
                        LEFT JOIN users u ON a.author_id = u.id 
                        WHERE (a.title_en LIKE ? OR a.title_ur LIKE ? OR a.content_en LIKE ? OR a.content_ur LIKE ?) 
                        AND a.status = 'published' 
                        ORDER BY a.created_at DESC
                        LIMIT ? OFFSET ?
                    ");
                    $stmt->bind_param("ssssii", $search_query_param, $search_query_param, $search_query_param, $search_query_param, $limit, $offset);
                    $stmt->execute();
                    $articles = $stmt->get_result();
                    if ($articles->num_rows == 0) {
                    ...
                    echo '</div>';
                    $total_articles_search_query = $conn->prepare("
                        SELECT COUNT(*) as count FROM articles 
                        WHERE (title_en LIKE ? OR title_ur LIKE ? OR content_en LIKE ? OR content_ur LIKE ?)
                        AND status = 'published'
                    ");
                    $total_articles_search_query->bind_param("ssss", $search_query_param, $search_query_param, $search_query_param, $search_query_param);
                    $total_articles_search_query->execute();
                    $total_articles_search = $total_articles_search_query->get_result()->fetch_assoc()['count'];
                    ...
                    if ($total_pages_search > 1) {
                        ...
                        for ($i = 1; $i <= $total_pages_search; $i++) {
                            $active = $i == $page ? 'active' : '';
                            echo '<li class="page-item ' . $active . '">';
                            echo '<a class="page-link" href="?view=search&q=' . urlencode($query) . '&page=' . $i . '&lang=' . $lang . '">' . $i . '</a>';
                            echo '</li>';
                        }
                        echo '</ul></nav>';
                    }
                    ...
                    if (hasRole('admin')) {
                        ...
                        if ($users->num_rows === 0) {
                            ...
                            while ($user = $users->fetch_assoc()) {
                                ...
                                echo '<td>';
                                if ($user['id'] !== $_SESSION['user_id']) { // Prevent admin from deleting self
                        ...
                        echo '</div>';
                        echo '</div>'; // End card
                        echo '</div>'; // End Backup/Restore Tab
                    }
                ...
                function include_profile_view()
                {
                    ...
                    // Display user's approved comments
                    echo '<div class="card mt-4 shadow-sm">';
                ...
                function include_breaking_news_view()
                {
                    ...
                    echo '</div>';
                    $total_articles_breaking_query = $conn->query("SELECT COUNT(*) as count FROM articles WHERE is_breaking = 1 AND status = 'published'");
                ...
                function include_archive_view()
                {
                    ...
                    $articles_query = "
                        ...
                        LEFT JOIN categories c ON a.category_id = c.id 
                        LEFT JOIN users u ON a.author_id = u.id 
                        WHERE YEAR(a.created_at) = ? AND MONTH(a.created_at) = ? AND a.status = 'published'
                        ORDER BY a.created_at DESC 
                        LIMIT ? OFFSET ?
                    ";
                    $stmt = $conn->prepare($articles_query);
                    ...
                    echo '</div>';
                    $total_articles_archive_query = $conn->prepare("SELECT COUNT(*) as count FROM articles WHERE YEAR(created_at) = ? AND MONTH(created_at) = ? AND status = 'published'");
    ...
    <script>
        ...
        function importData(input) {
            ...
            if (file) {
                ...
                fetch('', {
                    ...
                    .then(response => response.text()) // Read as text first to debug JSON errors
                    .then(text => {
                        try {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            if (document.documentElement.lang === 'ur') {
                const numberElements = document.querySelectorAll('.stats-number, .badge, .most-read-number, .view-count, .category-count, .archive-count');
        ...
        if (editArticleModal) {
            editArticleModal.addEventListener('show.bs.modal', event => {
                ...
                const imagePreview = editArticleModal.querySelector('#imagePreview');
                const removeImageCheckbox = editArticleModal.querySelector('#removeImageCheckbox');
                form.reset(); // Reset form validation and fields
        ...
        // Function to get Urdu month names
        function getUrduMonthName(monthNumber) {
            const urduMonths = {
                1: 'جنوری',
                2: 'فروری',
