                    ...
                    fetch(apiUrl)
                        ...
                        .then(data => {
                            if (data && data.current) {
                                ...
                                if (lang === 'ur') {
                                    html = `
                                <p class="mb-1" style="font-size: 1.2rem;"><strong>${convertToUrduNumbers(tempC)}${units.temperature_2m} - ${weatherInfo.ur}</strong></p>
                                <p class="mb-1"><small>محسوس: ${convertToUrduNumbers(feelsLike)}${units.apparent_temperature}</small></p>
                                <p class="mb-1"><strong>ہوا:</strong> ${convertToUrduNumbers(windSpeed)} ${units.windspeed_10m}</p>
                                <p class="mb-1"><strong>نمی:</strong> ${convertToUrduNumbers(humidity)}${units.relativehumidity_2m}</p>
                                <p class="mb-0"><strong>بارش:</strong> ${convertToUrduNumbers(precipitation)} ${units.precipitation}</p>
                            `;
                                } else {
                                    html = `
                                <p class="mb-1" style="font-size: 1.2rem;"><strong>${tempC}${units.temperature_2m} - ${weatherInfo.en}</strong></p>
                                <p class="mb-1"><small>Feels like: ${feelsLike}${units.apparent_temperature}</small></p>
                                <p class="mb-1"><strong>Wind:</strong> ${windSpeed} ${units.windspeed_10m}</p>
                                <p class="mb-1"><strong>Humidity:</strong> ${humidity}${units.relativehumidity_2m}</p>
                                <p class="mb-0"><strong>Precipitation:</strong> ${precipitation} ${units.precipitation}</p>
                            `;
                                }
                                weatherWidget.innerHTML = html + '<small class="text-muted mt-1 d-block">Data: Open-Meteo</small>';
                            } else {
                        ...
                        .catch((error) => {
                            ...
                            weatherWidget.innerHTML = `<p class="text-danger">${lang === 'ur' ? 'موسم کا ڈیٹا حاصل کرنے میں ناکامی۔' : 'Failed to get weather data.'}</p>`;
                        });
                }, () => {
                    weatherWidget.innerHTML = `<p class="text-warning">${lang === 'ur' ? 'مقام کی اجازت درکار ہے۔' : 'Location access denied.'}</p>`;
                });
            } else {
